...
# need to shuffle if doing hierachical softmax
shuf ../alldata-id.txt ../alldata-id.shuffled.txt
time ./word2vec -train ../alldata-id.shuffled.txt -output vectors.txt -cbow 0 -size 100 -window 10 -negative 5 -hs 1 -sample 1e-3 -threads 40 -binary 0 -iter 20 -min-count 1 -sentence-vectors 1
rm ../alldata-id.shuffled.txt
grep '_\*' vectors.txt | sort -n -k 2,2 -t '*' > sentence_vectors.txt
